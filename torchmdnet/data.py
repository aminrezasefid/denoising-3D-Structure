...
class DataModule(LightningDataModule):
    ...
    def setup(self, stage):
        if self.dataset is None: #if dataset id custom
            if self.hparams["dataset"] == "Custom":
            ...
            else: #if scale for noise is determined just scale the noise which is adding to data 
                if self.hparams['position_noise_scale'] > 0.:
                    ...
                    def transform(data):
                        ...
                        noise=np.random.randn(data.pos.shape[0],data.pos.shape[1]) * self.hparams["rdkit_noise_scale"]
                ...
                #create a function that call datasets module with their argument provided
